"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GetFeatureService = void 0;
const repositories_1 = require("../../../data/repositories");
const custom_error_1 = require("../../../domain/errors/custom.error");
class GetFeatureService {
    getAllFeatures() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield repositories_1.FeatureRepository.getAll();
            }
            catch (error) {
                throw custom_error_1.CustomError.internalServer(`${error}`);
            }
            ;
        });
    }
    ;
    getFeature(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const checkFeature = yield repositories_1.FeatureRepository.findById(id);
            if (!checkFeature) {
                throw custom_error_1.CustomError.badRequest('Feature does not exist');
            }
            ;
            try {
                return yield repositories_1.FeatureRepository.findById(id);
            }
            catch (error) {
                throw custom_error_1.CustomError.internalServer(`${error}`);
            }
        });
    }
    ;
}
exports.GetFeatureService = GetFeatureService;
;
